...
pub async fn handle_get_blobs(_name: String, hash: String) -> Result<impl Reply, Rejection> {
    ...
    if ! Path::new(&blob).is_file() {
        return Err(warp::reject::custom(registry_err {code: registry_err_code::BlobUnknown}));
    }
...
pub async fn handle_patch_blob(name: String, id: String, bytes: Bytes) -> Result<impl Reply, Rejection> {
...
pub async fn handle_put_blob(name: String, id: String, params: HashMap<String, String>, bytes: Bytes) -> Result<impl Reply, Rejection> {
    ...
    let digest = match params.get("digest") {
        Some(v) => v,
        None => return Err(warp::reject::custom(registry_err {code: registry_err_code::Unknown(String::from("missing digest"))}))
    };
