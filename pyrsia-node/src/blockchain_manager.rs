...
impl<'a> X509CertificateSignature<'a> {
    ...
    pub fn x509_certificate(&mut self) -> Result<X509Certificate<'a>, anyhow::Error> {
        match X509Certificate::from_der(self.x509_certificate_der).context("Unable to parse x509_certificate_der value ") {
            Ok(x509) => {
                self.x509_certificate_binary = Some(x509.1);
