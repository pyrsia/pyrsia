...
brew install pyrsia
```
- Open a terminal and try to start Pyrsia Node using the following
```shell
export RUST_LOG=info
pyrsia_node
```
This will start Pyrsia node in you local machine with `info` logs. (you can also skip `export RUST_LOG=info` if you
don't want logs. You can also set this to `export RUST_LOG=debug` to see even more logs)

When you start the Pyrsia node for first time, macOS might ask for network connection permission. So make sure to
allow this.

- Once Pyrsia node is running, open another terminal to verify everything is running smoothly.
...
pyrsia ping
```
Successful node inclusion in the network will display `Connection Successful !!`
