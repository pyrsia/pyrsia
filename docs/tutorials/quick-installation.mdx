...
In version 0.2 we chose to include a mininal Windows installer. It simply installs the node and cli executables that needs
...
- Run the installer - you will have to ignore the warning because we didn't sign our installer yet.
...
- The installer will now install Pyrsia (pyrsia_node.exe and pyrsia.exe) in `C:\Pyrsia` - we will make this configurable in future releases.
  You're free to move this to a better suited location after installation.
...
This will start your local Pyrsia node with `info` logs. (you can also skip this if you don't want logs, or set this to `debug` to see even more logs)
